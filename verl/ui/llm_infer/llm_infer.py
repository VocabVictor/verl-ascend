...
from verl.llm import DeployArguments, InferArguments, InferClient, InferRequest, RequestConfig
from verl.ui.base import BaseUI
from verl.ui.llm_infer.model import Model
from verl.ui.llm_infer.runtime import Runtime
...
class LLMInfer(BaseUI):
    ...
    def deploy(cls, *args):
        ...
        if sys.platform == 'win32':
            ...
            run_command = f'{cuda_param}start /b verl deploy {params} > {log_file} 2>&1'
        else:
            run_command = f'{cuda_param} nohup verl deploy {params} > {log_file} 2>&1 &'
