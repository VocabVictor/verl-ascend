...
from verl.ui.llm_train.utils import close_loop, run_command_in_subprocess
from verl.utils import TB_COLOR, TB_COLOR_SMOOTH, get_logger, read_tensorboard_file, tensorboard_smoothing
from verl.utils.utils import format_time
...
class Runtime(BaseUI):
    ...
    locale_dict = {
        ...
        'running_tasks': {
            ...
            'info': {
                'zh': '运行中的任务（所有的verl sft/pt命令）',
                'en': 'All running tasks(started by verl sft/pt)'
            }
        },
        'refresh_tasks': {
            'value': {
    ...
    def get_plot(cls, task):
        if not task or 'verl sft' in task or 'verl pt' in task:
            return cls.sft_plot
    ...
    def refresh_tasks(running_task=None, group=None):
        ...
        process_name = 'verl'
        negative_names = ['verl.exe', 'verl-script.py']
    ...
    def parse_info_from_cmdline(task):
        ...
        if '/cmd:' in task:
            for i in range(3):
                ...
                task = task[slash + 1:]
        if 'verl sft' in task:
            args = task.split('verl sft')[1]
        elif 'verl pt' in task:
            args = task.split('verl pt')[1]
        elif 'verl rlhf' in task:
            args = task.split('verl rlhf')[1]
        else:
    ...
    def cmd_to_sh_format(cmd):
        ...
        cmd_sh += (env_params + ' \\\n')
        verl_cmd = params[0].split('nohup')[1].strip()
        cmd_sh += ('nohup ' + verl_cmd + ' \\\n')
