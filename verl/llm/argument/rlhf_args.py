...
from verl.llm import MODEL_MAPPING
from verl.trainers.arguments import GRPOArgumentsMixin, RLHFArgumentsMixin
from verl.utils import get_logger, is_master, is_mp, set_default_ddp_config
...
class RLHFArguments(TeacherModelArguments, GRPOArguments, PPOArguments, RewardModelArguments, RLHFArgumentsMixin,
    ...
    def _init_external_vllm(self):
        if self.rlhf_type != 'grpo' or self.vllm_server_host is None:
            return
        from verl.trainers.rlhf_trainer.vllm_client import VLLMClient
        if is_master():
    ...
    def _check_grpo(self):
        ...
        if self.async_generate:
            ...
            'please deploy vLLM server by `verl rollout` and assign with `vllm_server_host` '
            'for more infomations, please check '
            'https://verl.readthedocs.io/en/latest/Instruction/GRPO/getstarted/GRPO.html'
    ...
    def _external_vllm_warning(self):
        ...
        if self.vllm_max_model_len is not None:
            logger.warning(
                ...
                '`verl rollout --max_model_len <value>`', self.vllm_max_model_len)
