...
from verl.llm import SamplingArguments, VerlPipeline, load_dataset
from verl.utils import get_logger

logger = get_logger()


class VerlSampling(VerlPipeline):
    ...
    def __init__(self, args: Union[List[str], SamplingArguments, None] = None) -> None:
        ...
        if self.args.sampler_type == 'sample':
            from verl.llm.sampling.vanilla_sampler import VanillaSampler
            self.sampler = VanillaSampler(self.args)
        elif self.args.sampler_type == 'mcts':
            from verl.llm.sampling.mcts import MctsSampler
            self.sampler = MctsSampler(self.args)
        elif self.args.sampler_type == 'distill':
            from verl.llm.sampling.distill_sampler import DistillSampler
