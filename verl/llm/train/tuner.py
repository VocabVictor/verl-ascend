...
from verl.llm import TrainArguments, deep_getattr, get_model_arch
from verl.plugin import Tuner, extra_tuners
from verl.tuners import Verl
from verl.utils import (activate_parameters, find_all_linears, find_embedding, find_norm, freeze_parameters,
...
def apply_liger(model_type: str):
    ...
    from verl.llm import ModelType
...
def prepare_adapter(args: TrainArguments, model, *, template=None, train_dataset=None, task_type=None):
    from verl.tuners import (AdaLoraConfig, AdapterConfig, BOFTConfig, LLaMAProConfig, LongLoRAModelType, LoraConfig,
                              LoRAConfig, ReftConfig, Verl, VeraConfig)
    ...
    if args.train_type in ('lora', 'longlora'):
        if args.use_verl_lora:
            lora_config = LoRAConfig(lora_dtype=args.lora_dtype, **lora_kwargs)
            model = Verl.prepare_model(model, lora_config)
        ...
        if args.train_type == 'longlora':
            ...
            from verl.tuners.longlora.llama import replace_llama_attn
        ...
        from verl.plugin.optimizer import calculate_max_steps
        ...
        model = Verl.prepare_model(model, adalora_config)
        ...
        model = Verl.prepare_model(model, llamapro_config)
        ...
        model = Verl.prepare_model(model, adapter_config)
        ...
        model = Verl.prepare_model(model, vera_config)
        ...
        model = Verl.prepare_model(model, boft_config)
        ...
        model = Verl.prepare_model(model, fourier_config)
        ...
        model = Verl.prepare_model(model, {'reft': reft_config})
        ...
        model = Verl.prepare_model(model, bone_config)
    return model
...
class TunerMixin:
    ...
    def prepare_model(cls, args, model, *, template=None, train_dataset=None, task_type=None):
        ...
        if args.use_galore:
            from verl.trainers.optimizers.galore import GaLoreConfig
