...
from verl.utils import get_dist_setting, get_env_args, get_logger, use_torchacc
...
class Template(ProcessorMixin):
    ...
    def __init__(
        ...
        *,
        ...
        template_backend: Literal['verl', 'jinja'] = 'verl',
    ) -> None:
        """
        ...
        """
        from .template_meta import TemplateMeta
        from verl.plugin import agent_templates, loss_scale_map
    ...
    def _pre_tokenize_images(self, context_list: List[Context], loss_scale_list: List[float],
                             inputs: StdTemplateInputs) -> Tuple[List[Context], List[float]]:
        # https://github.com/modelscope/ms-verl/issues/3407
        ...
        for context, loss_scale in zip(context_list, loss_scale_list):
            if context == '<image>' and inputs.is_multimodal and inputs.image_idx < len(inputs.images):
                ...
                inputs.image_idx += 1
                loss_scale = 0. if self.template_backend == 'verl' else 1.
            else:
                c_list = [context]
            res += c_list
    ...
    def _verl_prepare_messages(self, messages):
        if len(messages) < 2:
            return
        i = 1
        while i < len(messages):
    ...
    def _verl_encode(self, inputs: StdTemplateInputs):
        template_meta = self.template_meta
        system = self._get_system(inputs)
        self._verl_prepare_messages(inputs.messages)
    ...
    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        ...
        res_context_list, loss_scale_list, answer_len = (
            self._verl_encode(inputs) if template_backend == 'verl' else self._jinja_encode(inputs))
        encoded = {}
    ...
    def debug_logger(self, inputs):
        if not strtobool(os.getenv('VERL_DEBUG', 'false')):
            return
    ...
    def pre_forward_hook(self, model: nn.Module, args, kwargs):
        from verl.llm import to_device
    ...
    def data_collator(self, batch: List[Dict[str, Any]], *, padding_to: Optional[int] = None) -> Dict[str, Any]:
        from verl.llm import RowPreprocessor
        if self.mode == 'rlhf':
    ...
    def _torchacc_xtuner_data_collator(self, res, padding_to, tokenizer, padding_side):
        ...
        if use_torchacc():
            from verl.utils.torchacc_utils import pad_and_split_batch
