...
from verl.utils import get_hf_endpoint, use_hf_hub
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='verl/gpt4v-dataset',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='verl/RLAIF-V-Dataset',
...
class MantisPreprocessor(MessagesPreprocessor):
    ...
    def prepare_dataset(self, dataset: HfDataset) -> HfDataset:
        if not use_hf_hub():
            url = (f'https://www.modelscope.cn/api/v1/datasets/verl/Mantis-Instruct/repo?Revision='
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='verl/Mantis-Instruct',
...
class VideoChatGPTPreprocessor(ResponsePreprocessor):
    ...
    def prepare_dataset(self, dataset: HfDataset) -> HfDataset:
        if not use_hf_hub():
            url = 'https://modelscope.cn/datasets/verl/VideoChatGPT/resolve/master/videos.zip'
        else:
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='verl/VideoChatGPT',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='verl/Multimodal-Mind2Web',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='verl/MideficsDataset',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='verl/OK-VQA_train',
...
register_dataset(
    DatasetMeta(
        ms_dataset_id='verl/llava-instruct-mix-vsft',
