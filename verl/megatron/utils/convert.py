...
from verl.llm import ExportArguments, HfConfigFactory, get_model_tokenizer, get_template, save_checkpoint, to_device
from verl.utils import get_logger, get_n_params_grads
...
def test_convert_precision(hf_model, mg_model, processor, torch_dtype=torch.float32):
    ...
    inputs = template.encode({
        'messages': [
            {
                'role': 'user',
                'content': 'Introduction to ms-verl.'
            },
            {
                'role':
                'assistant',
                'content':
                'ms-verl is an official framework provided by the ModelScope community for fine-tuning '
...
def convert_mcore2hf(args: ExportArguments) -> None:
    from verl.megatron import prepare_mcore_model, adapter_state_dict_context
    kwargs = args.get_model_kwargs()
    hf_model, processor = get_model_tokenizer(**kwargs)
    if args.thread_count is None:
        checkpoint_size = sum(get_n_params_grads(hf_model)[0]) * torch.finfo(args.torch_dtype).bits // 8e9
