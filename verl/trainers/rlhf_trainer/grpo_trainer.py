...
from verl.llm import (InferRequest, MultiModelKeys, RequestConfig, RolloutInferRequest, RowPreprocessor, Template,
                       get_model_arch, to_device)
from verl.llm.infer.protocol import ChatCompletionResponse
from verl.llm.model.utils import get_llm_model
from verl.llm.template.base import MaxLengthError
from verl.llm.template.template_inputs import StdTemplateInputs
from verl.plugin import multi_turns, orms, rm_plugins
from verl.plugin.multi_turn import MultiTurnScheduler
from verl.utils import (JsonlWriter, empty_cache, get_current_device, get_device, get_logger, is_swanlab_available,
...
class GRPOTrainer(RLHFTrainerMixin, VerlMixin, HFGRPOTrainer):
    ...
    def __init__(self,
                 ...
                 *_args,
                 **kwargs):
        from verl.trainers.rlhf_arguments import GRPOConfig
        ...
        if reward_funcs:
            for i, reward_func in enumerate(reward_funcs):
                if reward_func in orms:
                    ...
                    raise ValueError(f'reward_function {reward_func} is not implemented in verl.llm.plugin')
        ...
        if reward_model is not None:
            ...
            for rm, rm_plugin, rm_template in zip(reward_model, reward_plugins, reward_template):
                ...
                if rm_plugin not in rm_plugins:
                    raise ValueError(f'rm_plugin {rm_plugin} is not implemented in verl.llm.plugin')
    ...
    def prepare_vllm(self, model):
        from verl.tuners import Verl
        from verl.llm.infer.infer_engine import GRPOVllmEngine
        max_num_seqs = (
            self.args.per_device_train_batch_size * self.vllm_tensor_parallel_size * self.args.steps_per_generation)
        current_device = get_device()
        with Verl.grpo_context(model, self.template.processor):
    ...
    def _apply_chat_template_to_messages_list(self, messages_list: InputsType):
        ...
        for messages in messages_list:
            ...
            res_context_list, _, _ = self.template._verl_encode(template_inputs)
