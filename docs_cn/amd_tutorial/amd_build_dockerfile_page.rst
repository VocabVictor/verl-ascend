...
作者：`Yusheng Su <https://yushengsu-thu.github.io/>`_

设置
-----

如果您在 AMD GPU (MI300) 上使用 ROCM 平台运行，则无法使用之前的快速入门来运行 verl。您应该按照以下步骤构建 docker，并在 verl 的 RLHF 训练中启动 ray 时设置 ``RAY_EXPERIMENTAL_NOSET_ROCR_VISIBLE_DEVICES`` 或 ``RAY_EXPERIMENTAL_NOSET_HIP_VISIBLE_DEVICES``。
...
.. code-block:: bash
    ...
    # docker images # 您可以找到构建的 docker

    # 支持 - 训练：fsdp；推理：vllm
    # FROM rocm/vllm:rocm6.2_mi300_ubuntu20.04_py3.9_vllm_0.6.4
    # 支持 - 训练：fsdp；推理：vllm、sglang
...
（可选）：如果您不想使用root模式并需要将自己指定为用户
请在上述docker启动脚本中添加 ``-e HOST_UID=$(id -u)`` 和 ``-e HOST_GID=$(id -g)``。

示例
-------

由于 AMD (ROCM) torch 的特殊设置，
1. 如果您的 ``ray>=2.45.0``（默认），您需要在 verl 的 RLHF 训练中启动 ray 时设置 ``RAY_EXPERIMENTAL_NOSET_HIP_VISIBLE_DEVICES``。
2. 如果您的 ``ray<2.45.0``，您需要在 verl 的 RLHF 训练中启动 ray 时设置 ``RAY_EXPERIMENTAL_NOSET_ROCR_VISIBLE_DEVICES``。
推理 ``$ENGINE`` 可以是 ``vllm`` 或 ``sglang``。在以下示例中我们选择 ``vllm`` 作为默认值。
...
.. code-block:: bash
    ...
    # [ray] >= 2.45.0
    export RAY_EXPERIMENTAL_NOSET_HIP_VISIBLE_DEVICES=1 # 使用 https://github.com/ray-project/ray/pull/52794 补丁
...
多节点训练：使用 Docker/Podman 容器的 slurm
---------------------------------------------------------------------------------------

如果您想使用 slurm 运行多节点训练，可以使用以下脚本。

.. note::
    1. 您需要在以下脚本中使用 ``podman`` 或 ``docker``。我们稍后会发布 apptainer 脚本。
...
.. code-block:: bash
    ...
    if [[ ${#ADDR[0]} -gt 16 ]]; then
        ...
        echo "检测到IPV6地址。我们将IPV4地址分离为 $head_node_ip"
    fi
    ...
    echo "在 $head_node 启动HEAD"
    srun --nodes=1 --ntasks=1 -w "$head_node" \
        docker exec "${CONTAINER_NAME}" \
            ray start --head --node-ip-address="$head_node_ip" --port=$port \
            --dashboard-port=8266 \
            --num-cpus "${SLURM_CPUS_PER_TASK}" --num-gpus "${SLURM_GPUS_PER_NODE}" --block &
    # 可选，但在某些版本的Ray < 1.0中可能有用。
    sleep 10
    ...
    for ((i = 1; i <= worker_num; i++)); do
        ...
        echo "调试：在 node_i = ${node_i} 启动worker"
        if [ -z "$node_i" ]; then
    ...
    # Ray初始化测试（查看上述执行中是否有任何错误）
    echo "在slurm节点中测试Ray初始化..."
