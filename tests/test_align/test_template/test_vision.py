...
def test_got_ocr():
    # https://github.com/modelscope/ms-verl/issues/2122
...
def test_got_ocr_hf():
    ...
    response = _infer_model(
        ...
        images=['https://modelscope-open.oss-cn-hangzhou.aliyuncs.com/images/ocr.png'])
    assert response[:200] == ('简介 VERL支持250+LLM和35+MLLM（多模态大模型）的训练、推理、 评测和部署。开发者可以直接将'
...
if __name__ == '__main__':
    from verl.llm import PtEngine, RequestConfig
    from verl.utils import get_logger, seed_everything
