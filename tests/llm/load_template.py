...
def load_and_tokenize(ms_model_id, template):
    from verl.llm import EncodePreprocessor, get_model_tokenizer, get_template
    try:
        ...
        if 'audio' in template_ins.__class__.__name__.lower():
            ...
            for row in load_ds('modelscope/coco_2014_caption:validation'):
                ...
                # output = EncodePreprocessor(template_ins)(load_ds('verl/OK-VQA_train'))
...
def load_and_tokenize_old(ms_model_id, template):
    ...
    from verl.llm import get_template, MODEL_MAPPING
    found = False
