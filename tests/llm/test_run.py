...
from verl import Trainer, TrainingArguments, get_logger
from verl.llm import (InferArguments, ModelType, RLHFArguments, TrainArguments, infer_main, merge_lora, rlhf_main,
                       sft_main)
...
class TestRun(unittest.TestCase):
    ...
    def test_hf_hub(self):
        ...
        train_dataset_fnames = [
            'alpaca.csv', 'chatml.jsonl', 'verl_pre.jsonl', 'verl_single.csv', 'verl_multi.jsonl',
            'verl_multi.json#2'
        ]
    ...
    def test_custom_dataset(self):
        ...
        train_dataset_fnames = [
            'alpaca.csv', 'chatml.jsonl', 'verl_pre.jsonl', 'verl_single.csv', 'verl_multi.jsonl',
            'verl_multi.json', 'sharegpt.jsonl'
        ]
        val_dataset_fnames = [
            'alpaca.jsonl',
            'alpaca2.csv',
        ...
        for num_train_epochs in [1, 2]:
            sft_args = TrainArguments(
                ...
                dataset=['verl/self-cognition#20'] + [os.path.join(folder, fname) for fname in train_dataset_fnames],
    ...
    def test_rlhf(self):
        ...
        for rlhf_type in visual_rlhf_types:
            for model in test_model:
                dataset_name = 'verl/RLAIF-V-Dataset#100'
                output = rlhf_main(
                    RLHFArguments(
                        rlhf_type=rlhf_type,
                        model=model,
    ...
    def test_loss_matching(self):
        ...
        if not __name__ == '__main__':
            ...
            return
        losses = []
        for use_verl_lora in [False, True]:
            bool_var = use_verl_lora
            torch.cuda.empty_cache()
            output = sft_main([
                ...
                '--seed', '0', '--lora_bias', 'all', '--modules_to_save', 'lm_head', '--use_verl_lora',
                str(use_verl_lora), '--num_train_epochs', '1', '--gradient_accumulation_steps', '16'
            ])
            best_model_checkpoint = output['best_model_checkpoint']
            print(f'best_model_checkpoint: {best_model_checkpoint}')
    ...
    def test_pai_compat(self):
        if not __name__ == '__main__':
            # ignore citest error in github
            return
        from verl.llm import sft_main, infer_main
