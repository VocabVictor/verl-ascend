...
from verl.llm import (DatasetMeta, InferRequest, Model, ModelGroup, ModelMeta, PtEngine, RequestConfig,
...
class TestCustom(unittest.TestCase):
    ...
    def test_custom_model(self):
        ...
        verl_response = response[0].choices[0].message.content

        engine.default_template.template_backend = 'jinja'
        response = engine.infer([infer_request], request_config)
        jinja_response = response[0].choices[0].message.content
        assert verl_response == jinja_response, (f'verl_response: {verl_response}\njinja_response: {jinja_response}')
        print(f'response: {verl_response}')

    def test_custom_dataset(self):
        dataset = load_dataset(['verl/stsb'])[0]
