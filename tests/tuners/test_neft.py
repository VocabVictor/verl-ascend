...
from verl.tuners import NEFTuneConfig
...
class TestNEFT(unittest.TestCase):
    ...
    def test_neft(self):
        ...
        model = Verl.prepare_model(model, config)
        model.train()
        ...
        model_new_2 = Verl.from_pretrained(model_new, self.tmp_dir)
    ...
    def test_neft_lora(self):
        ...
        t1 = model.embeddings.word_embeddings(inputs['input_ids'])
        model = Verl.prepare_model(model, {'c1': config, 'c2': config2})
        model.train()
        t2 = model.embeddings.word_embeddings(inputs['input_ids'])
        ...
        model_new = Verl.from_pretrained(model_new, self.tmp_dir)
        model_new.train()
