...
from verl import ResTuningConfig, Verl, VerlModel


class TestVerlResTuning(unittest.TestCase):
    ...
    def test_verl_restuning_vit(self):
        model_dir = snapshot_download('AI-ModelScope/vit-base-patch16-224')
        from transformers import AutoModelForImageClassification
        model = AutoModelForImageClassification.from_pretrained(model_dir)
        model_verl_1 = copy.deepcopy(model)
        model_verl_2 = copy.deepcopy(model)
        result_origin = model(torch.ones((1, 3, 224, 224))).logits
        print(f'test_verl_restuning_vit result_origin shape: {result_origin.shape}, '
        ...
        model_verl_1 = Verl.prepare_model(model_verl_1, config=restuning_config_1)
        self.assertTrue(isinstance(model_verl_1, VerlModel))
        print(model_verl_1.get_trainable_parameters())
        result_verl_1 = model_verl_1(torch.ones((1, 3, 224, 224))).logits
        print(f'test_verl_restuning_vit result_verl_1 shape: {result_verl_1.shape}, '
              f'result_verl_1 sum: {torch.sum(result_verl_1)}')
        ...
        model_verl_2 = Verl.prepare_model(model_verl_2, config=restuning_config_2)
        self.assertTrue(isinstance(model_verl_2, VerlModel))
        print(model_verl_2.get_trainable_parameters())
        result_verl_2 = model_verl_2(torch.ones((1, 3, 224, 224))).logits
        print(f'test_verl_restuning_vit result_verl_2 shape: {result_verl_2.shape}, '
              f'result_verl_2 sum: {torch.sum(result_verl_2)}')

        self.assertTrue(all(torch.isclose(result_verl_1, result_verl_2).flatten()))

        model_verl_1.save_pretrained(self.tmp_dir)
        self.assertTrue(os.path.exists(os.path.join(self.tmp_dir, 'default')))
        model_loaded = Verl.from_pretrained(model, self.tmp_dir)
        self.model_comparison(model_verl_1, model_loaded)

    def test_verl_restuning_diffusers_sd(self):
        ...
        print(f'test_verl_restuning_diffusers_sd result_origin shape: {result_origin.shape}, '
              f'result_origin sum: {torch.sum(result_origin)}')
        ...
        model = Verl.prepare_model(model, config=restuning_config)
        self.assertTrue(isinstance(model, VerlModel))
        print(model.get_trainable_parameters())

        result = model(**input_data).sample
        print(f'test_verl_restuning_diffusers_sd result shape: {result.shape}, result sum: {torch.sum(result)}')
