...
from verl import LoRAConfig, Verl
from verl.tuners.utils import ModulesToSaveWrapper
...
class TestExtraStateDict(unittest.TestCase):
    ...
    def test_verl_extra_state_dict(self):
        model = Model.from_pretrained('damo/nlp_structbert_sentence-similarity_chinese-base')
        lora_config = LoRAConfig(target_modules=['query', 'key', 'value'])
        model = Verl.prepare_model(model, lora_config, extra_state_keys=['classifier.*'])
        ...
        model = Model.from_pretrained('damo/nlp_structbert_sentence-similarity_chinese-base')
        model = Verl.from_pretrained(model, self.tmp_dir, inference_mode=False)
    ...
    def test_verl_modules_to_save(self):
        model = Model.from_pretrained('damo/nlp_structbert_sentence-similarity_chinese-base')
        ...
        model = Verl.prepare_model(model, {'lora1': lora_config, 'lora2': lora_config2})
        ...
        with open(os.path.join(self.tmp_dir, 'lora2', 'adapter_model.bin'), 'wb') as f:
            torch.save(state_dict, f)
        model = Model.from_pretrained('damo/nlp_structbert_sentence-similarity_chinese-base')
        model = Verl.from_pretrained(model, self.tmp_dir, adapter_name='lora2')
        names = [name for name, value in model.named_parameters() if value.requires_grad]
        self.assertTrue(any('classifier' in name for name in names))
        self.assertTrue(
